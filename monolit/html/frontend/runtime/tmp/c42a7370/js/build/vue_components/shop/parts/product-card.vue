<template>
  <div class="lg-grid-4 sm-grid-6 mc-grid-6 padded-sides">
    <div class="product_preview product_preview--product_list">
      <button type="button" class="block_wirl_fav is-added" v-if="favourite" v-on:click="setFavourite">
      ...
      <button type="button" class="block_wirl_fav not-added" v-if="!favourite" v-on:click="setFavourite">
      ...
      <div class="product_preview-prices prices" v-if="product.discount != 0">
        <span class="prices-old">{{ product.discount_sum }} грн.</span>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getFavourite() {
        axios.get("/shop/favourite/get/" + this.product.id).then((response) => {
          if(JSON.parse(response.data)[0].id) this.favourite = true;
        });
      }
    },
    created() {
